...
export const SCROLLBAR_FADE_MS = 280

export const SCROLLBAR_OPT_OUT_CLASS = 'no-scrollbar'
/** Keep the browser scrollbar visible instead of hiding it after idle. */
export const SCROLLBAR_PERSISTENT_CLASS = 'vcb-scrollbar-persistent'
/** Opt an ordinary scrolling element into the themed native scrollbar. */
export const SCROLLBAR_AUTO_CLASS = 'vcb-scrollbar-auto'
/** Show the native thumb while the user is actively scrolling. */
export const SCROLLBAR_VISIBLE_CLASS = 'is-scrollbar-visible'

const SCROLLBAR_NEVER_CLASSES = [
  SCROLLBAR_OPT_OUT_CLASS,
  SCROLLBAR_PERSISTENT_CLASS
] as const
...
export type ScrollbarRevealController = {
  reveal: (element: HTMLElement) => void
  syncTrackedScrollbars: () => void
  dispose: () => void
}

/**
 * Reveals the browser-owned scrollbar while scrolling, then fades its native
 * thumb back to transparent after the idle delay.
 */
export function createScrollbarRevealController(
  hideDelayMs = SCROLLBAR_HIDE_DELAY_MS
): ScrollbarRevealController {
